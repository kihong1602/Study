...
<div class="container">
    <form action="<c:url value="/new-member/admin-delete"/>" method="post">
        <table class="table">
            <thead>
            <tr>
                <th scope="col">프로필</th>
            ...
            <c:forEach items="${requestScope.userList}" var="user" varStatus="status">
                ...
                <tr>
                    <th scope="row"><img
                            src="${pageContext.request.contextPath}/upload/${user.profile}" alt=""
                            class="customImg">
                    </th>
                    ...
                    <td>
                        <input type="checkbox" class="check" name="check" value="${user.no}">
                    </td>
                </tr>
                </tbody>
            </c:forEach>
    ...
    <form action="<c:url value="/new-member/search"/>" method="post">
...
<script>
  ...
  $('.btnDelete').on("click", function () {
    ...
    $.ajax({
      url: "/new-member/admin-delete-all",
      data: {
        userNo: $(this).data("no"),
      },
